      ...
      <template v-slot:top>
            ...
            <v-card>
              ...
              <v-card-title class="headline" v-if="adAccion==2">
                ...
                <v-card-title class="headline" v-if="adAccion==3">
                  Aprobar Solicitud en Ausencia de APITAB
              </v-card-title>

                 <v-card-title class="headline" v-if="adAccion==4">
                 Servicio Realizado
              </v-card-title>
               <v-card-title class="headline" v-if="adAccion==5">
                 Servicio No Realizado
              </v-card-title>

              <v-card-text>
                  Estas a punto de <span v-if="adAccion==1">Aprobar</span>
                  <span v-if="adAccion==2">Cancelar</span> <span v-if="adAccion==3">Aprobar</span> <span v-if="adAccion==4">Finalizar</span><span v-if="adAccion==5">Finalizar</span> la solicitud con folio SU{{adNombre}}
              </v-card-text>
        ...
        ></v-textarea>
              ...
              <v-card-actions>
                ...
                <v-spacer></v-spacer>
                <v-btn @click="activarDesactivarCerrar()"  small color="orange">
                  Salir
                </v-btn>
                ...
                <v-btn v-if="adAccion==5" @click="finalizarServicioNoRealizado()" small color="success">
                  Finalizar Solicitud
                </v-btn>
...
<br>
       ...
       <div v-if="estado==0">
                            ...
                            Aprobar Solicitud 
         <v-btn  @click="activarDesactivarMostrar(1,{_id},{folioId})" class="mx-2" fab dark small color="primary">
      <v-icon dark>check</v-icon>
    </v-btn>         Cancelar Solicitud 
                          
         <v-btn  @click="activarDesactivarMostrar(2,{_id},{folioId})" class="mx-2" fab dark small color="error">
      ...
      <v-icon dark>block</v-icon>
         ...
         <v-btn  @click="activarDesactivarMostrar(3,{_id},{folioId})" class="mx-2" fab dark small color="success">
      <v-icon dark>check</v-icon>
    </v-btn>       
                            </div>
   ...
   <div v-if="estado==3">
         ...
         <v-btn  @click="activarDesactivarMostrar(4,{_id},{folioId})" class="mx-2" fab dark small color="success">
      <v-icon dark>save</v-icon>
         ...
         <v-btn  @click="activarDesactivarMostrar(5,{_id},{folioId})" class="mx-2" fab dark small color="red">
      <v-icon dark>report</v-icon>
    </v-btn>       
                            </div>
      ...
      return{
        ...
        methods: {
          ...
          ocultarNuevo(){
            ...
            console.log(me.agenciasUsuario)
      ...
      ventana.close(); //cerramos la ventana
        ...
        activar(){
          let me=this;
           let header={"Token":this.$store.state.token};
            var usuario=''
           let configuracion= {headers: header}
           axios.put('solicitudUnica/activate',{'_id':this._id,'aprobacionAmls':this.$store.state.usuario.nombre},configuracion)
          
               .then(function(response){
                
                me.listarEmbarcacion(response.data.embarcacion, response.data.procedencia,response.data.usuario.nombre,response.data.comentarios,moment(response.data.eta).format('MMMM Do YYYY, h:mm a'),moment(response.data.etd).format('MMMM Do YYYY, h:mm a'),response.data.muelle,status)
                swal(
         ...
         finalizarServicio(){
          let me=this;
           let header={"Token":this.$store.state.token};
            var usuario=''
           let configuracion= {headers: header}
           axios.put('solicitudUnica/finalizarServicio',{'_id':this._id,'aprobacionAmls':this.$store.state.usuario.nombre},configuracion)
          ...
          finalizarServicioNoRealizado(){
          let me=this;
           let header={"Token":this.$store.state.token};
            var usuario=''
           let configuracion= {headers: header}
           axios.put('solicitudUnica/finalizarServicioNoRealizado',{'_id':this._id,'aprobacionAmls':this.$store.state.usuario.nombre},configuracion)
          
               .then(function(response){
                
                swal(
                  "Solicitud Finalizada",
                  "La solicitud se ha guardado como no realizada",
                  "success"
                )
                me.ocultarComprobante()
                 me.adModal=0,
         ...
         aprobarApi(item){
          ...
          let cancelacion=''
           let header={"Token":this.$store.state.token};
           let configuracion= {headers: header}
           axios.put('solicitudUnica/aprobarApi',{'_id':this._id,'aprobacionApi':this.$store.state.usuario.nombre},configuracion)
               .then(function(response){
                 ...
                 me.ocultarComprobante()
                  swal(
                  "Solicitud Confirmada",
                  "Solicitud confirmada con exito por ausencia de APITAB",
                  "success"
                )
                 me.adModal=0,
                 me.adAccion=0,
        ...
        desactivar(){
          let me=this;
           let header={"Token":this.$store.state.token};
           let configuracion= {headers: header}
           axios.put('solicitudUnica/deactivate',{'_id':this._id,'aprobacionAmls':this.$store.state.usuario.nombre,'motivo_cancelacion':this.motivo_cancelacion},configuracion)
               .then(function(response){
