<template>
  <v-layout align-start>
   
  <v-flex>
        <h2>Reportes</h2>

        <!--Modulo de reporte-->
        <v-col class="d-flex" cols="12" sm="6">
            <v-select
            :items="tipo_reporte"
            label="Seleccionar Tipo de Reporte"
            v-model="tipoReporte"
            v-on:change="serviciosChange()"
            ></v-select>
        </v-col>
        <!-- Tipo de reporte -->
        <v-col class="d-flex" cols="12" sm="6" v-if="tipoReporte=='Embarque de Personal'">
            <v-select
            :items="reporteCG"
            label="Seleccionar Tipo de Reporte"
            v-model="reporteCategoria"
            v-on:change="reporteServiciosChange()"
            ></v-select>
        </v-col>
        <v-col class="d-flex" cols="12" sm="6" v-if="tipoReporte=='Solicitud Unica de Acceso'">
            <v-select
            :items="reporteSUA"
            label="Seleccionar Tipo de Reporte"
            v-model="reportEmb"
            ></v-select>
        </v-col>
          <v-col class="d-flex" cols="12" sm="6" v-if="tipoReporte=='Lista de Personal'">
            <v-select
            :items="reporteP"
            label="Seleccionar Tipo de Reporte"
            v-model="reportePersonal"
            ></v-select>
        </v-col>
         <v-col class="d-flex" cols="12" sm="6" v-if="tipoReporte=='Accesos a Vehiculos y Personal'">
            <v-select
            :items="reporteAVP"
            label="Seleccionar Tipo de Reporte"
            v-model="reportVeh"
            ></v-select>
        </v-col>    
           <v-col class="d-flex" cols="12" sm="6" v-if="tipoReporte=='Agencias'">
            <v-select
            :items="reporteAgencias"
            label="Seleccionar Tipo de Reporte"
           
            ></v-select>
        </v-col> 
          <v-col class="d-flex" cols="12" sm="6" v-if="tipoReporte=='Embarcaciones'">
            <v-select
            :items="reporteEmbarcaciones"
            label="Seleccionar Tipo de Reporte"
           
            ></v-select>
        </v-col>   
        <!-- Filtros -->
          <v-container>
            <v-row>
                    <!--Filtros de Reportes de cambios de guardia -->
                    <v-col cols="4" sm="4" md="4" v-if="reporteCategoria=='Por Usuario' || reporteCategoria=='Por Fecha' || reporteCategoria=='Por Embarcacion' || reporteCategoria=='Por Estatus'  || reporteCategoria=='Por Muelle' || reporteCategoria=='Servicios Por Tipo' || reporteCategoria=='Servicios Por Muelle'">
                        <v-datetime-picker label="Fecha Inicial" locale="es" v-model="fecha1"> </v-datetime-picker>
                    </v-col>
                       <v-col cols="4" sm="4" md="4" v-if="reporteCategoria=='Por Usuario' || reporteCategoria=='Por Fecha'  || reporteCategoria=='Por Embarcacion' || reporteCategoria=='Por Estatus' || reporteCategoria=='Por Muelle' || reporteCategoria=='Servicios Por Tipo' || reporteCategoria=='Servicios Por Muelle'">
                        <v-datetime-picker label="Fecha Final" locale="es" v-model="fecha2"> </v-datetime-picker>
                    </v-col>
                    <v-col cols="4" sm="4" md="4" v-if="reporteCategoria=='Por Usuario'">
                        <v-autocomplete  v-model="usuario" :items="usuarios"  label="Usuarios"></v-autocomplete>
                    </v-col>
                 
                    <v-col cols="4" sm="4" md="4" v-if="reporteCategoria=='Por Embarcacion'">
                        <v-autocomplete  v-model="embarcacion" :items="embarcaciones"  label="Embarcacion"></v-autocomplete>
                    </v-col>
                     <v-col cols="4" sm="4" md="4" v-if="reporteCategoria=='Servicios Por Agencia'">
                        <v-autocomplete  v-model="agencia" :items="agencias"  label="Agencia"></v-autocomplete>
                    </v-col>
                      <v-col cols="4" sm="4" md="4" v-if="reporteCategoria=='Por Estatus'">
                        <v-autocomplete  v-model="estado" :items="estados"  label="Estado"></v-autocomplete>
                    </v-col>
                      <v-col cols="4" sm="4" md="4" v-if="reporteCategoria=='Por Muelle'">
                        <v-autocomplete  v-model="muelle" :items="muelles"  label="Muelle"></v-autocomplete>
                    </v-col>
                      <v-col cols="4" sm="4" md="4" v-if="reporteCategoria=='Servicios Por Muelle'">
                        <v-text-field v-model="muelle" label="Ingresa el nombre del muelle"></v-text-field>
                      </v-col>

                      <!--Filtros de reportes de solicitudes Unicas -->
                   
                        <v-col cols="4" sm="4" md="4" v-if="reportEmb=='Por Solicitante' || reportEmb=='Por Agencia' || reportEmb=='Por Fecha' || reportEmb=='Por Muelle' || reportEmb == 'Por Embarcacion' || reportEmb == 'Por Estatus'">
                        <v-datetime-picker label="Fecha Inicial" locale="es" v-model="fecha1"> </v-datetime-picker>
                    </v-col>
                       <v-col cols="4" sm="4" md="4" v-if="reportEmb=='Por Solicitante' || reportEmb=='Por Agencia' || reportEmb=='Por Fecha' || reportEmb=='Por Muelle' || reportEmb=='Por Embarcacion' || reportEmb == 'Por Estatus'" >
                        <v-datetime-picker label="Fecha Final" locale="es" v-model="fecha2"> </v-datetime-picker>
                    </v-col>
                        <v-col cols="4" sm="4" md="4" v-if="reportEmb=='Por Solicitante'">
                        <v-autocomplete :items="usuarios" v-model="usuario" label="Usuario"></v-autocomplete>
                      </v-col>
                      <v-col cols="4" sm="4" md="4" v-if="reportEmb=='Por Agencia'">
                        <v-autocomplete :items="agencias" v-model="agencia" label="Agencias"></v-autocomplete>
                      </v-col>
                      <v-col cols="4" sm="4" md="4" v-if="reportEmb=='Por Muelle'">
                        <v-autocomplete  v-model="muelle" :items="muelles"  label="Muelle"></v-autocomplete>
                    </v-col>
                      <v-col cols="4" sm="4" md="4" v-if="reportEmb=='Por Embarcacion'">
                        <v-autocomplete  v-model="embarcacion" :items="embarcaciones"  label="Embarcacion"></v-autocomplete>
                    </v-col>
                      <v-col cols="4" sm="4" md="4" v-if="reportEmb=='Por Estatus'">
                        <v-autocomplete  v-model="estado" :items="estatus"  label="Estatus"></v-autocomplete>
                    </v-col>

                     <!--Filtros de reportes lista de personal -->

                          <v-col cols="4" sm="4" md="4" v-if="reportePersonal=='Personal que Embarco' || reportePersonal=='Personal que Desembarco'">
                        <v-datetime-picker label="Fecha Inicial" locale="es" v-model="fecha1"> </v-datetime-picker>
                    </v-col>
                       <v-col cols="4" sm="4" md="4" v-if="reportePersonal=='Personal que Desembarco' || reportePersonal=='Personal que Embarco'" >
                        <v-datetime-picker label="Fecha Final" locale="es" v-model="fecha2"> </v-datetime-picker>
                    </v-col>
                       <v-col cols="4" sm="4" md="4" v-if="reportePersonal=='Personal que Embarco'">
                        <v-autocomplete  v-model="embarcacion" :items="embarcaciones"  label="Embarcacion"></v-autocomplete>
                    </v-col>

                    <!--Filtros de reportes de vehiculos y personal -->



                     <v-col cols="4" sm="4" md="4" v-if="reportVeh=='Por Usuario' || reportVeh=='Por Agencia' || reportVeh=='Por Fecha' || reportVeh=='Por Muelle' || reportVeh == 'Por Embarcacion' || reportVeh == 'Por Estatus'">
                        <v-datetime-picker label="Fecha Inicial" locale="es" v-model="fecha1"> </v-datetime-picker>
                    </v-col>
                       <v-col cols="4" sm="4" md="4" v-if="reportVeh=='Por Usuario' || reportEmb=='Por Agencia' || reportEmb=='Por Fecha' || reportEmb=='Por Muelle' || reportEmb=='Por Embarcacion' || reportEmb == 'Por Estatus'" >
                        <v-datetime-picker label="Fecha Final" locale="es" v-model="fecha2"> </v-datetime-picker>
                    </v-col>
                      <v-col cols="4" sm="4" md="4" v-if="reportVeh=='Por Usuario'">
                        <v-autocomplete :items="usuarios" v-model="usuario" label="Usuario"></v-autocomplete>
                      </v-col>


            </v-row>
          </v-container>

     
    <v-btn
      :loading="loading"
      :disabled="loading"
      small
      color="blue-grey"
      class="ma-2 white--text"
      @click="loader = 'loading'"
    >
      Generar Reporte
      <v-icon right dark>print</v-icon>
    </v-btn>

         <v-dialog v-model="comprobanteModal" max-width="1700px">
            <v-card>
              <v-card-title class="headline" >
                  <v-btn @click="imprimir()"><v-icon>print </v-icon> </v-btn> Reportes
              </v-card-title>
              <v-card-text>
                   <div id="solicitud"> <br><br>
                                  <header>
                            <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css" integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous">
                          <div id="logo">
                              <img src="data:image/png;base64," id="imagen">
                          </div>
                        
                        
                          <br>
                          <div >

                          </div>  
                          <br>
                          <div id="fact">           
                            Carmen, Campeche. a:{{fecha}}<br><br>
                           
                          </div>
                  </header>
                  
                                             <strong v-if="reporteCategoria=='Por Usuario' || reporteCategoria=='Por Fecha'">Reportes de Cambios de Guardia del {{fechaR1}} a {{fechaR2}} </strong>
                                             <strong v-if="reporteCategoria=='Por Embarcacion'" >Reportes de Cambios de Guardia del {{fechaR1}} a {{fechaR2}} con la Embarcación {{embarcacionDetalle}}</strong>
                                             <strong v-if="reporteCategoria=='Por Estado'" >Reportes de Cambios de Guardia del {{fechaR1}} a {{fechaR2}} con estatus {{estado}}</strong>
                                             <strong v-if="reporteCategoria=='Por Muelle'" >Reportes de Cambios de Guardia del {{fechaR1}} a {{fechaR2}} en el muelle {{muelle}}</strong>

                  
                            <div style="" >

                           <div class="row" v-if="reporteCategoria=='Por Usuario'">
                        <div class="col-md-8">
                          <table class="table table-sm">
                          
                            <tbody>
                              <tr>   
                                <td>Nombre</td>
                                <td><strong>{{usuarioDetalle.nombre}}</strong></td> 
                                <td>Agencia</td>
                                <td><strong>{{usuarioAgencia.nombre}}</strong></td>
                              </tr>
                              <tr>
                                <td>Dirección</td>
                                <td><strong>{{usuarioDetalle.direccion}}</strong></td>
                                <td>RFC</td>
                                <td><strong>{{usuarioAgencia.rfc}}</strong></td>
                              </tr>
                              <tr> 
                                <td>Email</td>
                                <td><strong>{{usuarioDetalle.email}}</strong></td>
                                <td>Representante</td>
                                <td><strong>{{usuarioAgencia.representante}}</strong></td>
                              </tr>
                              <tr> 
                               <td>Telefono</td>
                                <td><strong>{{usuarioDetalle.telefono}}</strong></td>
                                <td></td>
                                <td><strong></strong></td>
                              </tr>
                            
                            </tbody>
                          </table>
                        </div>
                    </div>
                            </div>
                            
                     <div class="row">
                      <div class="col-md-12">
                         

                             <table class="table table-sm"  v-if="reporteCategoria=='Por Usuario'">
                             
                              <thead>
                                <tr>
                                  <th scope="col">#</th>
                                  <th scope="col">Embarcacion</th>
                                  <th scope="col">Muelle</th>
                                  <th scope="col">Detalles</th>
                                  <th scope="col">Fecha</th>
                                  <th scope="col">Folio</th>
                                  <th scope="col">Al servicio de la compañia</th>
                             
                                  
                                  
                                </tr>
                              </thead>
                              <tbody v-for="(det, index) in cambiosGuardia" :key="det._id">
                                 <tr >   
                                          <td>{{index+1}}</td>
                                          <td>{{det.embarcacion.nombre}}</td>
                                          <td>{{det.muelle}}</td>
                                          <td>{{det.detalles}}</td>
                                          <td>{{det.fecha.join(" al ")}}</td>
                                          <td>CG{{det.folio}}</td>
                                          <td>{{det.compania}}</td>
                                  </tr>
                                  
                              </tbody>
                             
                                
                            </table>

                              <table class="table table-sm"  v-if="reporteCategoria=='Por Fecha' || reporteCategoria=='Por Embarcacion'"> 
                             
                              <thead>
                                <tr>
                                  <th scope="col">#</th>
                                  <th scope="col">Usuario</th>
                                  <th scope="col">Embarcación</th>
                                  <th scope="col">Muelle</th>
                                  <th scope="col">Detalles</th>
                                  <th scope="col">Fecha</th>
                                  <th scope="col">Folio</th>
                                  <th scope="col">Al servicio de la compañia</th>

                             
                                  
                                  
                                </tr>
                              </thead>
                              <tbody v-for="(det, index) in cambiosGuardia" :key="det._id">
                                 <tr >   
                                          <td>{{index+1}}</td>
                                          <td>{{det.usuario[0].nombre}}</td>
                                          <td>{{det.embarcacion.nombre}}</td>
                                          <td>{{det.muelle}}</td>
                                          <td>{{det.detalles}}</td>
                                          <td>{{det.fecha.join(" al ")}}</td>
                                          <td>CG{{det.folio}}</td>
                                          <td>{{det.compania}}</td>
                                  </tr>
                                  
                              </tbody>
                             
                                
                            </table>
                             <table class="table table-sm"  v-if="reporteCategoria=='Por Estatus'"> 
                             
                              <thead>
                                <tr>
                                  <th scope="col">#</th>
                                  <th scope="col">Usuario</th>
                                  <th scope="col">Embarcacion</th>
                                  <th scope="col">Muelle</th>
                                  <th scope="col">Detalles</th>
                                  <th scope="col">Fecha</th>
                                  <th scope="col">Folio</th>
                                  <th scope="col">Al servicio de la compañia</th>
                             
                                  
                                  
                                </tr>
                              </thead>
                              <tbody v-for="(det, index) in cambiosGuardia" :key="det._id">
                                 <tr >   
                                          <td>{{index+1}}</td>
                                       
                                          <td>{{det.usuario[0].nombre}}</td>
                                          <td>{{det.embarcacion.nombre}}</td>
                                          <td>{{det.muelle}}</td>
                                          <td>{{det.detalles}}</td>
                                          <td>{{det.fecha.join( " al " )}}</td>
                                          <td>CG{{det.folio}}</td>
                                          <td>{{det.compania}}</td> 
                                        
                                  </tr>
                                  
                              </tbody>
                             
                                
                            </table>
                             <table class="table table-sm"  v-if="reporteCategoria=='Por Muelle'"> 
                             
                              <thead>
                                <tr>
                                 <th scope="col">#</th>
                                  <th scope="col">Usuario</th>
                                  <th scope="col">Embarcacion</th>
                                  <th scope="col">Muelle</th>
                                  <th scope="col">Detalles</th>
                                  <th scope="col">Fecha</th>
                                  <th scope="col">Folio</th>
                                  <th scope="col">Al servicio de la compañia</th>
                             
                                  
                                  
                                </tr>
                              </thead>
                              <tbody v-for="(det, index) in cambiosGuardia" :key="det._id">
                                 <tr >   
                                         <td>{{index+1}}</td>
                                       
                                          <td>{{det.usuario[0].nombre}}</td>
                                          <td>{{det.embarcacion.nombre}}</td>
                                          <td>{{det.muelle}}</td>
                                          <td>{{det.detalles}}</td>
                                          <td>{{det.fecha.join( " al " )}}</td>
                                          <td>CG{{det.folio}}</td>
                                          <td>{{det.compania}}</td> 
                                        
                                  </tr>
                                  
                              </tbody>
                             
                                
                            </table>
                             <table class="table table-sm"  v-if="reportEmb=='Por Solicitante'"> 
                             
                              <thead>
                                <tr>
                                  <th scope="col">#</th>
                                  <th scope="col">Usuario</th>
                                  <th scope="col">Agencia</th>
                                  <th scope="col">Embarcación</th>
                                  <th scope="col">Muelle</th>
                                  <th scope="col">Procedencia</th>
                                  <th scope="col">Estado</th>
                                  <th scope="col">Detalles</th>
                             
                                  
                                  
                                </tr>
                              </thead>
                              <tbody v-for="(det, index) in cambiosGuardia" :key="det._id">
                                 <tr >   
                                          <td>{{index+1}}</td>
                                          <td>{{det.usuario[0].nombre}}</td>
                                          <td>{{det.usuario[0].agencia.nombre}}</td>
                                          <td>{{det.embarcacion.nombre}}</td>
                                          <td>{{det.muelle}}</td>
                                          <td>{{det.procedencia}}</td>
                                          <td v-if="det.estado==0">En Proceso</td>
                                          <td v-if="det.estado==1">Aprobada por AMLS</td>
                                          <td v-if="det.estado==3">Aprobada por APITAB</td>
                                          <td v-if="det.estado==2">Cancelada</td>
                                           <td>{{det.comentarios}}</td>
                                        
                                  </tr>
                                  
                              </tbody>
                             
                                
                            </table>

                                 <table class="table table-sm"  v-if="reportEmb=='Por Agencia'"> 
                             
                              <thead>
                                <tr>
                                  <th scope="col">#</th>
                                  <th scope="col">Usuario</th>
                                  <th scope="col">Correo</th>
                                  <th scope="col">Embarcación</th>
                                  <th scope="col">Muelle</th>
                                
                                  
                                  <th scope="col">Detalles</th>
                             
                                  
                                  
                                </tr>
                              </thead>
                              <tbody v-for="(det, index) in cambiosGuardia" :key="det._id">
                                 <tr >   
                                          <td>{{index+1}}</td>
                                          <td>{{det.usuario.nombre}}</td>
                                          <td>{{det.usuario.email}}</td>
                                          <td>{{det.embarcacion.nombre}}</td>
                                          <td>{{det.muelle}}</td>
                                         
                                           
                                           <td>{{det.detalles}}</td>
                                        
                                  </tr>
                                  
                              </tbody>
                             
                                
                            </table>

                             <table class="table table-sm"  v-if="reportEmb=='Por Fecha'"> 
                             
                              <thead>
                                <tr>
                                  <th scope="col">#</th>
                                  <th scope="col">Usuario</th>
                                  <th scope="col">Agencia</th>
                                  <th scope="col">Embarcación</th>
                                  <th scope="col">Muelle</th>
                                  <th scope="col">Procedencia</th>
                                  <th scope="col">Estado</th>
                                  <th scope="col">Detalles</th>
                             
                                  
                                  
                                </tr>
                              </thead>
                              <tbody v-for="(det, index) in cambiosGuardia" :key="det._id">
                                 <tr >   
                                          <td>{{index+1}}</td>
                                          <td>{{det.usuario[0].nombre}}</td>
                                          <td>{{det.usuario[0].agencia.nombre}}</td>
                                          <td>{{det.embarcacion.nombre}}</td>
                                          <td>{{det.muelle}}</td>
                                          <td>{{det.procedencia}}</td>
                                          <td v-if="det.estado==0">En Proceso</td>
                                          <td v-if="det.estado==1">Aprobada por AMLS</td>
                                          <td v-if="det.estado==3">Aprobada por APITAB</td>
                                          <td v-if="det.estado==2">Cancelada</td>
                                           <td>{{det.comentarios}}</td>
                                        
                                  </tr>
                                  
                              </tbody>
                             
                                
                            </table>

                             <table class="table table-sm"  v-if="reportEmb=='Por Muelle'"> 
                             
                              <thead>
                                <tr>
                                  <th scope="col">#</th>
                                  <th scope="col">Usuario</th>
                                  <th scope="col">Agencia</th>
                                  <th scope="col">Embarcación</th>
                                  <th scope="col">Muelle</th>
                                  <th scope="col">Procedencia</th>
                                  <th scope="col">Estado</th>
                                  <th scope="col">Detalles</th>
                             
                                  
                                  
                                </tr>
                              </thead>
                              <tbody v-for="(det, index) in cambiosGuardia" :key="det._id">
                                 <tr >   
                                          <td>{{index+1}}</td>
                                          <td>{{det.usuario[0].nombre}}</td>
                                          <td>{{det.usuario[0].agencia.nombre}}</td>
                                          <td>{{det.embarcacion.nombre}}</td>
                                          <td>{{det.muelle}}</td>
                                          <td>{{det.procedencia}}</td>
                                          <td v-if="det.estado==0">En Proceso</td>
                                          <td v-if="det.estado==1">Aprobada por AMLS</td>
                                          <td v-if="det.estado==3">Aprobada por APITAB</td>
                                          <td v-if="det.estado==2">Cancelada</td>
                                           <td>{{det.comentarios}}</td>
                                        
                                  </tr>
                                  
                              </tbody>
                             
                                
                            </table>

                              <table class="table table-sm"  v-if="reportEmb=='Por Embarcacion'"> 
                             
                              <thead>
                                <tr>
                                  <th scope="col">#</th>
                                  <th scope="col">Usuario</th>
                                  <th scope="col">Agencia</th>
                                  <th scope="col">Embarcación</th>
                                  <th scope="col">Muelle</th>
                                  <th scope="col">Procedencia</th>
                                  <th scope="col">Estado</th>
                                  <th scope="col">Detalles</th>
                             
                                  
                                  
                                </tr>
                              </thead>
                              <tbody v-for="(det, index) in cambiosGuardia" :key="det._id">
                                 <tr >   
                                          <td>{{index+1}}</td>
                                          <td>{{det.usuario[0].nombre}}</td>
                                          <td>{{det.usuario[0].agencia.nombre}}</td>
                                          <td>{{det.embarcacion.nombre}}</td>
                                          <td>{{det.muelle}}</td>
                                          <td>{{det.procedencia}}</td>
                                          <td v-if="det.estado==0">En Proceso</td>
                                          <td v-if="det.estado==1">Aprobada por AMLS</td>
                                          <td v-if="det.estado==3">Aprobada por APITAB</td>
                                          <td v-if="det.estado==2">Cancelada</td>
                                           <td>{{det.comentarios}}</td>
                                        
                                  </tr>
                                  
                              </tbody>
                             
                                
                            </table>

                              <table class="table table-sm"  v-if="reportEmb=='Por Estatus'"> 
                             
                              <thead>
                                <tr>
                                  <th scope="col">#</th>
                                  <th scope="col">Usuario</th>
                                  <th scope="col">Agencia</th>
                                  <th scope="col">Embarcación</th>
                                  <th scope="col">Muelle</th>
                                  <th scope="col">Procedencia</th>
                                  <th scope="col">Estado</th>
                                  <th scope="col">Detalles</th>
                             
                                  
                                  
                                </tr>
                              </thead>
                              <tbody v-for="(det, index) in cambiosGuardia" :key="det._id">
                                 <tr >   
                                          <td>{{index+1}}</td>
                                          <td>{{det.usuario[0].nombre}}</td>
                                          <td>{{det.usuario[0].agencia.nombre}}</td>
                                          <td>{{det.embarcacion.nombre}}</td>
                                          <td>{{det.muelle}}</td>
                                          <td>{{det.procedencia}}</td>
                                          <td v-if="det.estado==0">En Proceso</td>
                                          <td v-if="det.estado==1">Aprobada por AMLS</td>
                                          <td v-if="det.estado==3">Aprobada por APITAB</td>
                                          <td v-if="det.estado==2">Cancelada</td>
                                           <td>{{det.comentarios}}</td>
                                        
                                  </tr>
                                  
                              </tbody>
                             
                                
                            </table>


                         
                      </div>
                     </div>

                        <div class="row">
                      <div class="col-md-8">
                        
                      
                         
                      </div>
                     </div>

                  </div>
                  
              </v-card-text>
              <v-card-actions>
                <v-btn @click="ocultarComprobante()" small>Cancelar</v-btn>
              </v-card-actions>
            </v-card>

          </v-dialog>
  </v-flex>
 </v-layout>
</template>
<script>

import axios from 'axios';
import DatetimePicker from 'vuetify-datetime-picker'
import jsPDF from 'jspdf'
import html2canvas from 'html2canvas'

  export default {
     icons: {
    iconfont: 'fa4',
  },
    data(){
      return{
                  dialog: false,
                  search: '',
                 monitoreo:[],
                headers: [
               { text: 'Actions', value: 'action', sortable: false},
               { text: 'Estado', value: 'estado', sortable:true},
               { text: 'Muelle', value: 'servicio.muelle', sortable:true},
               { text: 'Programa', value: 'servicio.programa', sortable:true},
             
            ],
            
            editedIndex: -1,
            _id:'',
            embarcacion:'',
            embarcaciones:[],
            agencias:[],
            agencia:'',
            fecha:'',
            colores:['cyan','green','pink','amber','orange'],
            color:'',
            servicios:[],
            servicio:'',
            reportePersonal:'',
            reporteP:['Personal que Embarco', 'Personal que Desembarco'],
            reporteCategoria:'',
            usuarioDetalle:[],
            reportCat:'',
            loader: null,
            loading: false,
            tipo_reporte:['Solicitud Unica de Acceso','Embarque de Personal','Lista de Personal','Accesos a Vehiculos y Personal','Embarcaciones','Agencias','Usuarios'],
            reporteCG:['Por Usuario','Por Fecha','Por Embarcacion','Por Estatus', 'Por Muelle','Por Agencia'],
            reporteSUA:['Por Agencia','Por Solicitante','Por Fecha','Por Estatus','Por Muelle','Por Embarcacion',],
            reporteAVP:['Por Usuario','Por Fecha','Por Embarcacion','Por Estatatus','Por Agencia','Por Muelle'],
            reporteAgencias:['Agencias Activas','Agencias Inactivas'],
            reporteEmbarcaciones:['Embarcaciones Activas','Embarcaciones Inactivas'],
            tipo:'',
            agencias:[],
            nombre:'',
            agencia2:'',
            cabeceraDetalles:[
              { text: 'Borrar', value: 'borrar', sortable: false},
              { text: 'Color', value: 'color' },
              { text: 'Fecha y Hora', value: 'fecha' },
              { text: 'Tipo de Movimiento', value: 'tipoMovimiento' },
              { text: 'Detalles', value: 'detalles' },
           
            ],
            movimientos:[],
            verNuevo:0,
            usuario:'',
            tipoMovimiento:'',
            detalles:'',
            fecha:'',
            muelles:['FRONTERA','CHILTEPEC','SANCHEZ MAGALLANEZ'],
            estatus:['En Proceso','Aprobados AMLS','Aprobados APITAB','Cancelados'],
            folio:'',
            fecha1:'',
            fecha2:'',
            cambiosGuardia:[],
            estado:'',
            detalles:'',
            usuarioAgencia:[],
            compania:'',
            usuarios:[],
            reportVeh:'',
            muelle:'',
            fechaR1:'',
            agencia:'',
            agencias:[],
            embarcacionDetalle:[],
            estados:['En Proceso','Aprobados por APITAB','Aprobados por AMLS','Cancelados'],
            reportEmb:'',
            fechaR2:'',
            tipoReporte:'',
            tipoRep:'',
            valida:0,
            fecha:null,
            validaMensaje:[],
            adModal:0,
            comprobanteModal:0,
            adAccion:0,
            adNombre:'',
            adId:''
      }
    },
          computed: {
          formTitle () {
            return this.editedIndex === -1 ? 'Nueva Solicitud' : 'Editar Solicitud'
          }
        },

        watch: {
          dialog (val) {
            val || this.close()
          }
        },

        created () {
       this.selectUsuarios()
       this.selectEmbarcaciones()
       this.selectAgencias()
        },
        watch: {
      loader () {
        const l = this.loader
        this.mostrarComprobante()
        this[l] = !this[l]

        setTimeout(() => (this[l] = false), 1000)
         
        this.loader = null
       
      },
    },

        methods: {

        

       

          mostrarNuevo(){
            this.verNuevo =1;

          },
          ocultarNuevo(){
            this.verNuevo =0
            this.limpiar()
          },

      
       


       

     
          limpiar(){
            this._id=''
           
            this.usuario='',
            this.folio='',
            this.agencia='',
            this.fecha='',
            this.pasajeros=[],
            this.valida=0,
            this.validaMensaje=[],
            this.editedIndex=-1;
          },
         
        
           serviciosChange(){
             this.tipoRep = this.tipoReporte 
             this.reporteCategoria = ''
            },

            reporteServiciosChange(){
             this.reportCat = this.reporteCategoria

            },

        
        
          listarDetalles(){
            if(this.reporteCategoria=='Por Usuario'){
                let header={"Token":this.$store.state.token};
                let configuracion= {headers: header}
                let me=this;
                axios.get('cambioGuardia/listCGUsuario?usuario='+this.usuario+'&fecha1='+this.fecha1+'&fecha2='+this.fecha2,configuracion).then(function (response){
                me.cambiosGuardia=response.data;
                }).catch(function(error){
                  console.log(error)
                })

                this.detallesUsuarios()
            }
            else if(this.reporteCategoria=='Por Fecha'){
                let header={"Token":this.$store.state.token};
                let configuracion= {headers: header}
                let me=this;
                axios.get('cambioGuardia/listFecha?fecha1='+this.fecha1+'&fecha2='+this.fecha2,configuracion).then(function (response){
                me.cambiosGuardia=response.data;
                }).catch(function(error){
                  console.log(error)
                })
                
            }
             else if(this.reporteCategoria=='Por Embarcacion'){
                let header={"Token":this.$store.state.token};
                let configuracion= {headers: header}
                let me=this;
                axios.get('cambioGuardia/listCGEmbarcacion?embarcacion='+this.embarcacion+'&fecha1='+this.fecha1+'&fecha2='+this.fecha2,configuracion).then(function (response){
                me.cambiosGuardia=response.data;
                }).catch(function(error){
                  console.log(error)
                })

               this.detallesEmbarcacion()
            }
             else if(this.reporteCategoria=='Por Estatus'){
                let header={"Token":this.$store.state.token};
                let configuracion= {headers: header}
                let me=this;
                let status
                if(me.estado=='En Proceso'){
                  status=0
                }if(me.estado=='Aprobados por AMLS'){
                  status = 1
                }if(me.estado=='Aprobados por APITAB'){
                  status=3
                }if(me.estado=='Cancelados'){
                  status = 2
                }
                
                axios.get('cambioGuardia/listCGEstado?estado='+status+'&fecha1='+this.fecha1+'&fecha2='+this.fecha2,configuracion).then(function (response){
                me.cambiosGuardia=response.data;
                console.log(me.cambiosGuardia)
                }).catch(function(error){
                  console.log(error)
                })
            }
              else if(this.reporteCategoria=='Por Muelle'){
                let header={"Token":this.$store.state.token};
                let configuracion= {headers: header}
                let me=this;
                axios.get('cambioGuardia/listCGMuelle?muelle='+this.muelle+'&fecha1='+this.fecha1+'&fecha2='+this.fecha2,configuracion).then(function (response){
                me.cambiosGuardia=response.data;
                }).catch(function(error){
                  console.log(error)
                })
            }
            else if(this.reportEmb=="Por Solicitante"){
                let header={"Token":this.$store.state.token};
                let configuracion= {headers: header}
                let me=this;
                axios.get('solicitudUnica/listarSUPorUsuario?usuario='+this.usuario+'&fecha1='+this.fecha1+'&fecha2='+this.fecha2,configuracion).then(function (response){
                me.cambiosGuardia=response.data;
                console.log(me.cambiosGuardia)
                }).catch(function(error){
                  console.log(error)
                })
                this.detallesUsuarios()
            }
             else if(this.reportEmb=="Por Agencia"){
                let header={"Token":this.$store.state.token};
                let configuracion= {headers: header}
                let me=this;
                axios.get('solicitudUnica/listarSUPorAgencia?usuario='+'PACIFIC'+'&fecha1='+this.fecha1+'&fecha2='+this.fecha2,configuracion).then(function (response){
                me.cambiosGuardia=response.data;
                console.log(me.cambiosGuardia)
                }).catch(function(error){
                  console.log(error)
                })
            }
            else if(this.reportEmb=="Por Fecha"){
                let header={"Token":this.$store.state.token};
                let configuracion= {headers: header}
                let me=this;
                axios.get('solicitudUnica/listarSUPorAgencia?&fecha1='+this.fecha1+'&fecha2='+this.fecha2,configuracion).then(function (response){
                me.cambiosGuardia=response.data;
                console.log(me.cambiosGuardia)
                }).catch(function(error){
                  console.log(error)
              })
            }
             else if(this.reportEmb=="Por Muelle"){
                let header={"Token":this.$store.state.token};
                let configuracion= {headers: header}
                let me=this;
                axios.get('solicitudUnica/listarSUPorMuelle?muelle='+this.muelle+'&fecha1='+this.fecha1+'&fecha2='+this.fecha2,configuracion).then(function (response){
                me.cambiosGuardia=response.data;
                console.log(me.cambiosGuardia)
                }).catch(function(error){
                  console.log(error)
                })
            }
             else if(this.reportEmb=="Por Embarcacion"){
                let header={"Token":this.$store.state.token};
                let configuracion= {headers: header}
                let me=this;
                axios.get('solicitudUnica/listarSUPorEmbarcacion?embarcacion='+this.embarcacion+'&fecha1='+this.fecha1+'&fecha2='+this.fecha2,configuracion).then(function (response){
                me.cambiosGuardia=response.data;
                console.log(me.cambiosGuardia)
                }).catch(function(error){
                  console.log(error)
                })
            }
             else if(this.reportEmb=="Por Estatus"){
                let header={"Token":this.$store.state.token};
                let configuracion= {headers: header}
                let me=this;
                let estado=''
                if(me.estado=='En Proceso'){
                  estado=0
                } if(me.estado=='Aprobados por AMLS'){
                  estado=1
                } if(me.estado=='Aprobados por APITAB'){
                  estado=3
                } if(me.estado=='Cancelados'){
                  estado=2
                }
                console.log(estado)
                axios.get('solicitudUnica/listarSUPorEstatus?estado='+estado+'&fecha1='+this.fecha1+'&fecha2='+this.fecha2,configuracion).then(function (response){
                me.cambiosGuardia=response.data;
                console.log(me.cambiosGuardia)
                }).catch(function(error){
                  console.log(error)
                })
            }
            
            
          },
           detallesUsuarios(){
            let header={"Token":this.$store.state.token};
            let configuracion= {headers: header}
            let me=this;
            axios.get('usuario/query?_id='+this.usuario,configuracion).then(function (  response){
            me.usuarioDetalle=response.data;
            me.usuarioAgencia=response.data.agencia
            console.log(me.usuarioDetalle)
            }).catch(function(error){
              console.log(error)
            })
          },
          detallesEmbarcacion(){
            let header={"Token":this.$store.state.token};
            let configuracion= {headers: header}
            let me=this;
            axios.get('embarcacion/query?_id='+this.embarcacion,configuracion).then(function (response){
            me.embarcacionDetalle=response.data;
            console.log(me.embarcacionDetalle)
            }).catch(function(error){
              console.log(error)
            })
          },

          imprimir(){
 var objeto = document.getElementById("solicitud"); //obtenemos el objeto a imprimir
      var ventana = window.open("", "_blank"); //abrimos una ventana vacía nueva
      ventana.document.write(objeto.innerHTML); //imprimimos el HTML del objeto en la nueva ventana
      ventana.document.close(); //cerramos el documento
      ventana.print(); //imprimimos la ventana
      ventana.close(); //cerramos la ventana
          },

           mostrarComprobante(){
            var options = { year: 'numeric', month: 'long', day: 'numeric' };
            var fec = new Date();   
            this.fecha = fec.toLocaleString(); 

            this.fechaR1 = this.fecha1.toLocaleString(); 
            this.fechaR2 = this.fecha2.toLocaleString();
            this.listarDetalles(),
            this.comprobanteModal=1          
          },
           ocultarComprobante(item){
            this.comprobanteModal=0
          },

         selectEmbarcaciones(){
            let me = this;
            let embarcacionArray=[];
            let header={"Token":this.$store.state.token};
            let configuracion= {headers: header}
            axios.get('embarcacion/list',configuracion).then((response)=>{
              embarcacionArray=response.data;
              embarcacionArray.map(function(x){
                me.embarcaciones.push({text:x.nombre, value:x._id})
              });
            }).catch(function(error){
              console.log(error);
            })
          },

        selectAgencias() {
          let me = this;
          let agencaiArray = [];
          let header = { Token: this.$store.state.token };
          let configuracion = { headers: header };
          axios
            .get("agencia/listActivos", configuracion)
            .then(response => {
              agencaiArray = response.data;
              agencaiArray.map(function(x) {
                me.agencias.push({ text: x.nombre, value: x._id });
              });
              console.log(me.agencias)
            })
            .catch(function(error) {
              console.log(error);
            });
        },

          
       
          selectUsuarios(){
            let me = this;
            let usuarioArray=[];
            let header={"Token":this.$store.state.token};
            let configuracion= {headers: header}
            axios.get('usuario/list',configuracion).then((response)=>{
              usuarioArray=response.data;
              usuarioArray.map(function(x){
                me.usuarios.push({text:x.nombre, value:x._id})
              });
            }).catch(function(error){
              console.log(error);
            })
          },


        activarDesactivarCerrar(){
          this.adModal=0;
        },

          close () {
            this.dialog = false
          },

         
        }
  }
</script>
<style>
  #factura {
            padding: 20px;
            font-family: Arial, sans-serif;
            font-size: 16px ;
        }

        #logo {
            float: left;
            margin-left: 2%;
            margin-right: 2%;
        }
         #logo2 {
           
            margin-left: 80%;
            margin-right: 2%;
        }
        #table_agencia {
           margin-right: 10px;
        }
        #imagen {
            width: 100px;
        }
         #imagen2{
            width: 90px;
        }

        #fact {
            font-size: 14px;
           
            text-align: right;
        }   

        #datos {
            float: left;
            margin-top: 0%;
            margin-left: 15%;
            margin-right: 2%;
            /*text-align: justify;*/
        }

        #encabezado {
            text-align: center;
            margin-left: 10px;
            margin-right: 10px;
            font-size: 16px;
        }

        section {
            clear: left;
        }

      

        #cliente {
            text-align: left;
        }

        #facliente {
            width: 40%;
            border-collapse: collapse;
            border-spacing: 0;
            margin-bottom: 15px;
        }

        #fa {
            color: #FFFFFF;
            font-size: 14px;
        }

          #fa2 {
            color: #FFFFFF;
            font-size: 14px;
        }

        #facarticulo {
            width: 90%;
            border-collapse: collapse;
            border-spacing: 0;
            padding: 20px;
            margin-bottom: 15px;
            border: .5px solid #ddd;
        }

        #facarticulo thead {
            padding: 20px;
            background: #31467c;
            text-align: center;
            border-bottom: 1px solid #FFFFFF;
        }

         #solicitante {
            width: 60%;
            border-collapse: collapse;
            border-spacing: 1;
            padding: 20px;
            margin-bottom: 15px;
            border: .5px solid #ddd;
        }

         #solicitante thead {
            padding: 20px;
            background: #31467c;
            text-align: center;
            
            border-bottom: 1px solid #FFFFFF;
        }

        #gracias {
            text-align: center;
        }
        #tituloSolicitante{
          background-color: #808285;
          border: 1px solid #ddd;
          color:#FFFFFF
          }
        #tituloPasajeros{
          background-color: #808285;
          border: 1px solid #ddd;
          color:#FFFFFF
       }
</style>