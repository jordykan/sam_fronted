      ...
      <v-data-table :headers="headers" :items="pasajeros" :search="search" class="elevation-1">
        ...
        <template v-slot:top>
          <v-toolbar flat color="white">
            ...
            <v-toolbar-title>Mis Compañias</v-toolbar-title>
...
export default {
  ...
  methods: {
    ...
    selectCompanias() {
      ...
      let configuracion = { headers: header };
      axios
        .get("companias/list", configuracion)
        .then(response => {
          companiaArray = response.data;
          companiaArray.map(function(x) {
            me.companias.push({ text: x.nombre, value: x._id });
          });
        })
        .catch(function(error) {
          console.log(error);
