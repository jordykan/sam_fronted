    ...
    <v-flex>
      ...
      >
            ...
            <v-dialog v-model="adModal" max-width="290">
              <v-card>
                ...
                <v-card-text>
                  ...
                  <span v-if="adAccion==3">Aprobar por ausencia de API</span>
                ...
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn @click="activarDesactivarCerrar()" color="orange" small>Salir</v-btn>
                  <v-btn
                    v-if="adAccion==1"
                    @click="activar()"
                    small
              ...
              <v-card>
                ...
                <v-card-text>
                  ...
                  <div v-if="estado==0">
                    Aprobar Solicitud
                    <v-btn
                      @click="activarDesactivarMostrar(1,{_id},{folio})"
                      class="mx-2"
                      fab
                      dark
                      small
                    ...
                    </v-btn>Rechazar Solicitud
                    <v-btn
                      @click="activarDesactivarMostrar(2,{_id},{folio})"
                      class="mx-2"
                      fab
                      dark
                      small
                  ...
                  <div v-if="bandera==true && estado==1">
                    ...
                    <v-btn
                      @click="activarDesactivarMostrar(3,{_id},{folio})"
                      class="mx-2"
                      fab
                      dark
                      small
        ...
        <template v-slot:item.action="{ item }">
          <v-icon
            v-if="item.estado==0 || item.estado==1 || item.estado==2 || item.estado==3 || item.estado==4"
            class="mr-2"
      ...
      <v-container grid-list-sm class="pa-4 white" v-if="verNuevo">
        <v-layout row wrap>
          <v-flex xs12 sm4 lg4 x14>
            <v-text-field disabled v-model="folio" label="Folio"></v-text-field>
          </v-flex>
          ...
          <v-flex xs12 sm4 lg8 x14>
            <v-textarea v-model="detalles" disabled clearable="clearable" label="Detalles"></v-textarea>
          </v-flex>
            ...
            <template>
              ...
              >
                <template slot="items" slot-scope="props">
                  ...
                  <td class="text-xs-center">
                    <v-text-field v-model="props.items.nsss"></v-text-field>
                  </td>
                  <td class="text-xs-center">
                ...
                <template v-slot:item.nombreCompleto="{ item }">
                  <td class="text-xs-center">
                    <v-text-field disabled v-model="item.nombreCompleto"></v-text-field>
                  </td>
                </template>
                <template v-slot:item.identificacion="{ item }">
                  <td class="text-xs-center">
                    <v-text-field disabled v-model="item.identificacion"></v-text-field>
                  </td>
                </template>
                <template v-slot:item.libretaMar="{ item }">
                  <td class="text-xs-center">
                    <v-text-field disabled v-model="item.nss"></v-text-field>
                  </td>
                </template>
                <template v-slot:item.destino="{ item }">
                  <td class="text-xs-center">
                    <v-text-field disabled v-model="item.destino"></v-text-field>
                  </td>
                </template>
              </v-data-table>
            </template>
    ...
    aprobarApi(item) {
      ...
      axios
        ...
        .then(function (response) {
          ...
          swal(
            "Solicitud Aprobada",
            "Solicitud de Embarque y Desembarque de Personal aprobada por ausencia de APITAB",
            "success"
          );
          (me.adModal = 0),
            (me.adAccion = 0),
    ...
    activar() {
      ...
      axios
        ...
        .then(function (response) {
          ...
          swal(
            "Solicitud Aprobada",
            "Solicitud de Embarque y Desembarque de Personal aprobada con exito",
            "success"
          );
          (me.adModal = 0),
            (me.adAccion = 0),
    ...
    desactivar() {
      ...
      axios
        ...
        .then(function (response) {
          ...
          swal(
            "Solicitud Cancelada",
            "Solicitud de Embarque y Desembarque de Personal Cancelada con exito",
            "success"
          );
